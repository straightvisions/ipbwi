<?php
	...
	class ipbwi_sso extends ipbwi{
		...
		private function process(string $query){
			...
			if(is_array($decoded)){
				...
				if($status['http_code'] == 404){
					throw new Exception('<div class="ipbwi_api_error">IPBWI API Error: The CURL request <strong>'.$this->curl_url.'</strong> in module <strong>'.$this->module.'</strong> was not successful. Status Code: '.$this->curl_info['http_code'].'</div>');
					return false;
				}else{
					return $result;
				}
		...
		public function verifySettings(){
			$result		= $this->process($this->buildParameters(array('do' => 'verifySettings', 'ourKey' => trim(ipbwi_IPS_CONNECT_SLAVE_KEY))));
			
			if(!is_array($result)){
				echo '<h3>Request URI</h3>';
				echo '<a href="'.$this->curl_info['url'].'">'.$this->curl_info['url'].'</a>';
				echo '<h3>Request Primary IP</h3>';
				echo '<a href="'.$this->curl_info['primary_ip'].'">'.$this->curl_info['primary_ip'].'</a>';
				echo $result;
